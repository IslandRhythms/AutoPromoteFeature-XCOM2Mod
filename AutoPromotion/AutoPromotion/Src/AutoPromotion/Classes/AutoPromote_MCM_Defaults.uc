// This is an Unreal Script
class AutoPromote_MCM_Defaults extends Object config(AutoPromotion_DEFAULT);

var config int VERSION_CFG;
var config bool ONLYSQUADDIES;
var config bool ONLYVETS;
var config bool CHECKBARRACKS;
var config bool ENABLELOGGING;
var config bool SHOWPROMOTIONPOPUP;
var config bool USENAME;
var config bool IGNORECA;
var config bool RANKNOBUY;
var config bool BUYRANDOM;
